      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        ...
        {(product.weight != null || product.length != null || product.width != null || product.height != null) && (
          <div className="p-6 border-t border-gray-200">
            <h3 className="text-lg font-semibold mb-4">Габариты и вес (для доставки СДЭК)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <p className="text-sm text-gray-500">Вес</p>
                <p className="font-medium">{product.weight != null ? `${product.weight} г` : '—'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Длина</p>
                <p className="font-medium">{product.length != null ? `${product.length} мм` : '—'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Ширина</p>
                <p className="font-medium">{product.width != null ? `${product.width} мм` : '—'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Высота</p>
                <p className="font-medium">{product.height != null ? `${product.height} мм` : '—'}</p>
              </div>
            </div>
          </div>
        )}
