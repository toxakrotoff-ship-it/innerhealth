...
import { checkoutCartBodySchema } from '@/lib/validations/checkout-session'
import { CheckoutSessionNotFoundError, updateCheckoutCart } from '@/lib/checkout-tracking'
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const parsed = checkoutCartBodySchema.safeParse(raw)
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const updated = await updateCheckoutCart(id, owner, {
      cartSnapshot: items,
      cartItemsCount,
      ...rest,
    })
    ...
    console.error('[checkout/session/:id/cart] Failed to update cart snapshot:', error)
    return NextResponse.json(
