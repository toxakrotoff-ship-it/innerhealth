...
function splitSprintHeroTitleForAccent(title: string): { lead: string; tail: string } {
  ...
  return {
    lead: trimmed.slice(0, boundary),
    tail: trimmed.slice(boundary).replace(/^\s+/, ''),
  }
}
        ...
        <AdaptiveContainer maxWidth="full" className="relative z-10">
          <div className="space-y-6">
            <div className="relative isolate min-h-[clamp(30rem,calc(100dvh-8rem),40rem)] overflow-hidden rounded-[clamp(1rem,2.5vw,1.75rem)] border border-[#162542] bg-[#031021] shadow-[0_24px_80px_rgba(0,0,0,0.32)] md:min-h-[clamp(34rem,calc(100dvh-7rem),48rem)] md:rounded-3xl">
              <div className="absolute inset-x-0 top-0 h-[15rem] overflow-hidden sm:h-[18rem] md:hidden">
                <Image
              ...
              />
              <div
                className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(3,16,33,0.14)_0%,rgba(3,16,33,0.42)_32%,#031021_48%,#031021_100%)] md:bg-[linear-gradient(90deg,#031021_0%,rgba(3,16,33,0.98)_30%,rgba(3,16,33,0.74)_50%,rgba(3,16,33,0.2)_74%,rgba(3,16,33,0.78)_100%)]"
                aria-hidden
              />
              <div
                ...
                aria-hidden
              />
              <div className="relative z-20 flex min-h-[inherit] flex-col justify-start px-[clamp(1.5rem,6vw,5rem)] pb-[clamp(1.75rem,5vw,4.5rem)] pt-[clamp(12rem,40vw,16rem)] md:justify-center md:py-[clamp(1.75rem,5vw,4.5rem)]">
          ...
          <div className="relative overflow-hidden rounded-[clamp(1rem,2.5vw,1.75rem)] border border-[#22304F] bg-[#0C1730] p-[clamp(1rem,2.8vw,1.75rem)] md:rounded-3xl md:p-8">
            ...
            <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
              ...
              <Link
                href={reviewsCtaHref}
                className="inline-flex items-center justify-center rounded-full bg-[#7AA2FF] px-5 py-3 text-sm font-semibold text-slate-950 transition-colors hover:bg-[#9AB8FF] md:shrink-0"
              >
                {getBlockTextForBrand(
                  blocks,
                  'home',
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  {data.newsPosts.map((post) => (
                    <Link
                      key={post.id}
                      href={`/news/${post.slug}`}
                      className="group overflow-hidden rounded-2xl border border-[#1B2946] bg-[#0F172A] transition-all hover:border-[#7AA2FF] hover:shadow-[0_0_0_1px_rgba(122,162,255,0.28)]"
                    >
      ...
      <AdaptiveContainer maxWidth="full">
        <div className="space-y-6 rounded-2xl bg-[#060A14]">
          ...
          <div className="rounded-[clamp(1rem,2.5vw,1.75rem)] bg-linear-to-b from-slate-100/90 via-white to-slate-50/70 p-[clamp(1rem,2.8vw,1.75rem)] md:rounded-3xl md:p-8">
            ...
            <Link
              href="/faq"
              className="mt-4 inline-flex w-full items-center justify-center rounded-full bg-[#7AA2FF] px-5 py-3 text-sm font-semibold text-slate-950 transition-colors hover:bg-[#9AB8FF]"
            >
              {getBlockTextForBrand(
                blocks,
                'home',
