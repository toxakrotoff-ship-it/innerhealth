...
**Целевая модель «конец проекта»** (согласованные решения по продукту): один бэкенд, две изолированные витрины; смешанной корзины нет; ЮKassa — **разные shopId** по брендам; СДЭК — общий с заделом на разводку; боты — **разные** на бренд; отчёты в основном по бренду + отдельная выгрузка «с обеих витрин»; единый ЛК и общие страницы входа/регистрации. Подробно и чеклист пробелов с кодом: **[two-storefronts-architecture.md](./two-storefronts-architecture.md)**.
...
- **Две витрины — целевые доработки** (см. [two-storefronts-architecture.md](./two-storefronts-architecture.md)): явное поле бренда на заказе (`Order.brand` или эквивалент), UX регистрации «аккаунт уже есть в сети — войти?», выгрузки отчётности **с обоих брендов** одной кнопкой, при необходимости — юридические страницы целиком из админки по бренду.
