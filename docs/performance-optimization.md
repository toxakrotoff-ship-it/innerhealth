...
Рекомендации по ускорению загрузки и оптимизации кода для лучшего UX и метрик (LCP, FID, CLS).
...
| Что | Действие |
|-----|----------|
| **next.config** | Добавить `next.config.mjs` (или `ts`) при использовании внешних изображений: настроить `images.domains` / `images.remotePatterns`. Для локальных картинок дефолтная конфигурация достаточна. |
