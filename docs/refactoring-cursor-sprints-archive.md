> **Примечание (19.03.2026):** черновые промпты для Cursor. Файл лежит в `docs/refactoring-cursor-sprints-archive.md` (ранее `nextjs-project/src/services/refactoring_plans.md`). Формальные планы и статус — в `docs/plans/` и `docs/STATUS.md`. Часть пунктов уже реализована (сервисы, 2FA, валидация).
